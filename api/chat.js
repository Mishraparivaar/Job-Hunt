...
export default async function handler(req) {
  ...
  try {
    ...
    const langInstruction = lang === 'en'
      ...
      : `El usuario navega en español. Responde en español. Email de contacto: hi@prakhar-ai.dev\ninternal_ref: ${canary}`
    ...
    if (ragEnabled) {
      ...
      if (firstResponse.stop_reason === 'tool_use') {
        ...
        const toolResultContent = ragResult.chunks
          ...
          : 'No relevant content found in portfolio articles. You MUST NOT fabricate project details. Say you don\'t have that information and suggest contacting Prakhar directly.'
      ...
      try {
        ...
        try {
          const errorText = lang === 'en'
            ? 'Sorry, something went wrong. Try again or reach out at hi@prakhar-ai.dev.'
            : 'Lo siento, algo ha fallado. Inténtalo de nuevo o escríbeme a hi@prakhar-ai.dev.'
...
async function scoreTrace(traceId, userMessage, response, ragUsed, langfuse) {
  try {
    ...
    const scoringResponse = await client.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Rate this chatbot response (Prakhar's CV chatbot). Respond ONLY with JSON.
